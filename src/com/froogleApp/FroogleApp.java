...
public class FroogleApp {
	...
	public static int particionar(Termo aTermos[], int start, int end) {
		...
		int pivot = aTermos[end].repeticao; // variavel pivo
		int i = (start - 1);

		for (int j = start; j <= end - 1; j++) {
			// Se o elemento atual for menor que o pivô
			if (aTermos[j].repeticao < pivot) {
				i++; // índice de incremento de elemento menor
				int t = aTermos[i].repeticao;
				aTermos[i].repeticao = aTermos[j].repeticao;
				aTermos[j].repeticao = t;
			}
		}
		int t = aTermos[i + 1].repeticao;

		aTermos[i + 1].repeticao = aTermos[end].repeticao;
		aTermos[end].repeticao = t;
		return (i + 1);
	}
/**
 * 
